...
SlVector3 Joint::calculateInBoard(std::vector<Joint> &joints, int currentJoint) {
  double x = 0,y = 0,z = 0;
  while (currentJoint > 0) {
    Joint joint = joints[currentJoint];
    int parent = joint.getParent();
    Joint parentJointVector = joints[parent];
    x = x + parentJointVector.getX();
    y = y + parentJointVector.getY();
    z = z + parentJointVector.getZ();
    currentJoint = parent;
  }
  SlVector3 newJoint(x,y,z);
  return newJoint;
}

SlVector3 Joint::calculateOutBoard(std::vector<Joint> &joints, int currentJoint) {
  Joint joint = joints[currentJoint];
  double x = joint.getX(), y = joint.getY(), z = joint.getZ();
  while (currentJoint > 0) {
    int parent = joint.getParent();
    Joint parentJointVector = joints[parent];
    x = x + parentJointVector.getX();
